...
const VistaPrincipal = (props) => {
  ...
  function updateSearch(textoBuscador) {
    if (textoBuscador.length > 0) {
      ...
      const podcastFiltrados = props.listadoPodcast.filter((item) => {
        ...
        return (
          titulo.toUpperCase().indexOf(search) > -1 ||
          autor.toUpperCase().indexOf(search) > -1
        );
      });
  ...
  async function abrirDetallesPodcast(podcast) {
    ...
    if (podcastLocal) {
      if (
        parseFloat(podcastLocal.timestamp) >
    ...
    if (necesarioCargar) {
      ...
      podcast.cargarDatosPodcast().then(
        function (res) {
          ...
          props.navigation.navigate("vistaDetallesPodcast", {
            podcast: podcast,
          });
        },
        function (err) {
